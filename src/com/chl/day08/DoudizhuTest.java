...
public class DoudizhuTest {
    ...
    private static List<String> piker;
    ...
    public static void main(String[] args) {
        ...
        boolean flag = true;
        while (flag){
            //如果set无法打乱顺序，换为list使用Collections.shuffle()打乱顺序
            Collections.shuffle(piker);
            System.out.println(piker);
            dipai = new ArrayList<>();
            xipai();
            if (dipai.contains("♥3")) {
                xipai();
    ...
    private static void xipai() {
        for (int i = 0; i <=53 ; i++) {
            if (i > 50) {
                dipai.add(piker.get(i));
            }else{
                if(i%3==0){
                    player1.add(piker.get(i));
                }else if (i % 3 == 1){
                    player2.add(piker.get(i));
                }else {
                    player3.add(piker.get(i));
                }
            }
        }
    }
